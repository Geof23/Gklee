...
it points to"
  (let* ((entries (split-string (gklee-buffer-string trace) "\\*\\*\\*\\*\\*" t))
    ...
    (with-current-buffer b
      ...
      (dotimes (i vsize)
	(setq cur-line (car entries))
	(catch 'next
	  (if (string-match "\\([0-9]+\\):\\([0-9]+\\)\n\\([0-9]+\\):\\([^\\:]*\\):\\([[:graph:][:space:]]*\\)" cur-line)
	      (progn
		(setq blk (match-string 1 cur-line))
		;;remove me
		(setq thd (match-string 2 cur-line))
		(setq line (match-string 3 cur-line))
		(setq llvm (match-string 5 cur-line))
		(setq file (gklee-translate-path (match-string 4 cur-line)))
		(if (not file)
		    (throw 'next nil))
		;; (if (not llvm)
		;;     (setq llvm ""))
...
(defun gklee-find-real-line (beg-line &optional reverse)
...
NIL is returned."
  (let ((curPoint beg-line)
    ...
    (while (and (or (invisible-p beg-mark) (equal (length trace-line) 0)) (not eof))
      (setq line-watch (forward-line (if (not reverse)
					 1
				       -1)))
      (setq curPoint (point))
      (set-marker beg-mark curPoint)
...
(defun gklee-step-back ()
  "This function should only be called after gklee-start-step. It performs the action of a 
single step backwards through the trace file. It is very similiar to start step but with 
just enough differences to warrant it's own function (uncoloring lines updating variables etc)"
  (interactive)
  (let (beg-line
	beg-mark
	(end-mark (make-marker))
    ...
    (while (and (not found) (not beg-buffer))
      (setq beg-line (gklee-find-real-line (point) t))
      (if (not beg-line)
	  (setq beg-buffer t)
	(progn
	  (goto-char beg-line)
    ...
    (if beg-buffer
	(progn
	  (message "Beginning of trace buffer reached, exiting step mode")
	  (gklee-step-quit))
      (progn
	(if (get-text-property beg-line 'face)
	    (setq gklee-step-trace-error t))
...
differences to warrant it's own function (uncoloring lines updating variables etc)"
  (interactive)
  (let (beg-line
	beg-mark
	(end-mark (make-marker))

...
void ObjectState::makeSymbolicAsPublic() {
  makeSymbolic();
}

bool ObjectState::isByteKnownSymbolicAsPublic(unsigned offset) {
  return isByteKnownSymbolic(offset);
}

void ObjectState::setKnownSymbolicAsPublic(unsigned offset, Expr* value) {
  setKnownSymbolic(offset, value);
}

void ObjectState::printAsPublic() {
  print();
}

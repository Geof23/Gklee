...
void StatsTracker::stepInstruction(ExecutionState &es) {
  if (OutputIStats) {
    ...
    Instruction *inst = es.getPC()->inst;
    const InstructionInfo &ii = *es.getPC()->info;
    StackFrame &sf = es.stacks.front().back();
	...
	stats::uncoveredInstructions += (uint64_t)-1;
      ...
      // per thread coverage
      executor.bc_cov_monitor.stepInstruction(es, inst);
    }
  }
}
...
void StatsTracker::markBranchVisited(ExecutionState *visitedTrue, 
  ...
  if (OutputIStats) {
	...
	++fullBranches; 
	--partialBranches; 
	executor.bc_cov_monitor.coverFullBranch(visitedTrue);
      }
      else {
	++partialBranches; 
      }
	...
	++fullBranches; 
	--partialBranches; 
	executor.bc_cov_monitor.coverFullBranch(visitedFalse);
      }
      else {
	++partialBranches;
      }
...
void StatsTracker::updateStateStatistics(uint64_t addend) {
  for (std::set<ExecutionState*>::iterator it = executor.states.begin(),
    ...
    const InstructionInfo &ii = *state.getPC()->info;
    theStatisticManager->incrementIndexedValue(stats::states, ii.id, addend);
    if (UseCallPaths)
      state.stacks.front().back().callPathNode->statistics.incrementValue(stats::states, addend);
  }
}
...
void StatsTracker::computeReachableUncovered() {
  ...
  for (std::set<ExecutionState*>::iterator it = executor.states.begin(),
    ...
    for (ExecutionState::stack_ty::iterator sfIt = es->stacks.front().begin(),
           sf_ie = es->stacks.front().end(); sfIt != sf_ie; ++sfIt) {
      ExecutionState::stack_ty::iterator next = sfIt + 1;
      KInstIterator kii;

      if (next==es->stacks.front().end()) {
        kii = es->getPC();
      } else {
        kii = next->caller;
        ++kii;

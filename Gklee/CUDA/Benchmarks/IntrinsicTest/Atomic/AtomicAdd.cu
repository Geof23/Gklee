...
__global__
void atomicIntAddKernel(int *uA, int *uB, int *uC) {
  unsigned tid = threadIdx.x; uC[tid] = atomicAdd(uA+tid, uB[tid]); 
}

__global__
void atomicUnsignedAddKernel(unsigned *uA, unsigned *uB, unsigned *uC) {
  unsigned tid = threadIdx.x;
  uC[tid] = atomicAdd(uA+tid, uB[tid]); 
}

__global__
void atomicULLAddKernel(unsigned long long int *uA, unsigned long long int *uB, 
...
__global__
void atomicFloatAddKernel(float *fA, float *fB, float *fC) {
  unsigned tid = threadIdx.x;
  fC[tid] = atomicAdd(fA+tid, fB[tid]); 
}
...
int main(int argv, char **argc) {
  ...
  atomicIntAddKernel<<<1, NUM>>>(diA, diB, diC);

  atomicUnsignedAddKernel<<<1, NUM>>>(dA, dB, dC); 

  atomicULLAddKernel<<<1, NUM>>>(dullA, dullB, dullC); 

  atomicFloatAddKernel<<<1, NUM>>>(dfA, dfB, dfC); 
  ...
  printf("After atomic add intrinsic: \n");

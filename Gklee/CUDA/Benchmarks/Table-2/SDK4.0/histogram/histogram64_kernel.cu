...
__global__ void histogram64Kernel(uint *d_PartialHistograms, data_t *d_Data, uint dataCount){
    ...
    printf("threadIdx: %d, threadPos: %d, HISTOGRAM64_THREADBLOCK_SIZE: %d, HISTOGRAM64_BIN_COUNT: %d\n", threadIdx.x, threadPos, HISTOGRAM64_THREADBLOCK_SIZE, HISTOGRAM64_BIN_COUNT);

    //Initialize shared memory (writing 32-bit words)
    //#pragma unroll
    for(uint i = 0; i < (HISTOGRAM64_BIN_COUNT / 4); i++) {
        printf("threadIdx: %d, threadPos: %d\n", threadIdx.x, threadPos);
...
extern "C" void initHistogram64(void){
    ...
    cudaMalloc((void **)&d_PartialHistograms, MAX_PARTIAL_HISTOGRAM64_COUNT * HISTOGRAM64_BIN_COUNT * sizeof(uint));
}
...
extern "C" void histogram64(
    ...
    printf("The histogramCount: %d, HISTOGRAM64_THREADBLOCK_SIZE: %d, size of data_t: %d \n", histogramCount, HISTOGRAM64_THREADBLOCK_SIZE, sizeof(data_t));

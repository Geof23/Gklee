...
public:
  ...
  enum TravPurpose {   // the purpose of traversing the Expr structure
    MakeStrLen,
    MakeStrExpr,
    ComputeValue
  };
...
public:
  Expr() : refCount(0) { Expr::count++; ctype = GPUConfig::UNKNOWN; }
  ...
  // an alias of the expr; usually a simpler expression (e.g. ConstantExpr)
  Expr* alias;
 
  // add a method to tell whether an expression is symbolic 
  // without consulting the solver (which is the default method)
  virtual bool isSymbolic() { return false; }

  // convert string expressions to usual ones. 
  // All string expressions will be eliminated. 
...
public:
  ...
  // by Guodong
  bool isSymbolic() { return src->isSymbolic(); }
};
...
public:
  ...
  // by Guodong 
  // Enable arrays of different types 
  enum Kind {
    FixedLenKind,     // bit array of a fixed length
    VarLenKind,       // bit array of a variable length
    LenVarKind        // for the variable representing a length
  };
...
public:
  ...
  Array(const std::string &_name, uint64_t _size, 
    ...
    : kind(FixedLenKind), name(_name), size(_size), 
  ...
  static bool classof(const Array *) { return true; }
...
public:
  ...
  bool isSymbolic() {
    if (root->isSymbolicArray()) 
      return true;
    UpdateNode *p = (UpdateNode*) head;
    while (!p) {
      if (p->index->isSymbolic() || p->value->isSymbolic()) 
	return true;
      p = (UpdateNode*) (p->next);
    }
    return false;
...
public:
  ...
  // by Guodong
  bool isSymbolic() { 
    return (index->isSymbolic() || updates.isSymbolic()); 
  }
};
...
public:
  ...
  // by Guodong
  bool isSymbolic() { 
    return (cond->isSymbolic()  || 
	    trueExpr->isSymbolic() ||
	    falseExpr->isSymbolic());
  }
...
public:
  ...
  // by Guodong
  bool isSymbolic() { return expr->isSymbolic(); }
};
...
public:
  ...
  ref<Expr> resolveStrExpr(StrConstraint &constr, TravPurpose pur) {
    return NotExpr::create(expr->resolveStrExpr(constr, pur));
  };
...
public:                                                              \
    ...
    ref<Expr> resolveStrExpr(StrConstraint &constr,                  \
                             TravPurpose pur) {			     \
    return new _class_kind ## Expr(                                  \
		    left->resolveStrExpr(constr, pur),               \

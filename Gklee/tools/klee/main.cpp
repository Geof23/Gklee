...
namespace {
  ...
  CheckLevel("check-level",
	  cl::desc("The level of checking of concurrency bugs"),
	  cl::init(2));
...
void KleeHandler::processTestCase(const ExecutionState &state,
  ...
  if (!NoOutput) {
    ...
    // By Peng
    if (!performSuffix)
      id = success ? ++m_testIndex : m_testIndex;
    else 
      id = m_testIndex;

    if (!state.traceInfo.empty()) {
    ...
    if (success) {
      ...
      if (performSuffix) {
        strcat(performSuffix, ".ktest"); 
        if (!kTest_toFile(&b, getTestFilename(performSuffix, id).c_str())) {
...
static llvm::Module *linkWithUclibc(llvm::Module *mainModule) {
  ...
  // by Guodong: remove the requirement of specifying the absolute path of uClibc
  // mainModule = klee::linkWithLibrary(mainModule, 
  //                                    KLEE_UCLIBC "/lib/libc.a");
  mainModule = klee::linkWithLibrary(mainModule,
				     KLEE_DIR "/klee-uclibc/lib/libc.a");
  assert(mainModule && "unable to link with uclibc");

  // by Guodong, link uClibc++
  // mainModule = klee::linkWithLibrary(mainModule, 
  //                                    KLEE_UCLIBC "/lib/libc++.a");
  // assert(mainModule && "unable to link with uclibc++");
...
static void handleSpecifiedExternLibs(std::vector<std::string> &llvmLibSet, 
                                      std::vector<std::string> &nativeLibSet) {
  std::vector<std::string> libs;  
  std::istringstream f(LoadLibrary.c_str());
  std::string s;    
  ...
  for (unsigned i = 0; i < libs.size(); i++) {
    llvm::sys::Path libPath(libs[i]);
    if (libPath.isBitcodeFile()) {
      llvmLibSet.push_back(libs[i]);
    } else {
      nativeLibSet.push_back(libs[i]);
    }
  }
}

void configurateGPU() {
  GPUConfig::verbose = Verbose;
  // check concurrency bugs?
  GPUConfig::check_level = CheckLevel;
  std::cout << "Configuration: concurrency bug checking level is: "
            << GPUConfig::check_level << std::endl;
...
int main(int argc, char **argv, char **envp) {  
  ...
  Gklee::Logging Log( "trace.log", LogLevel, false ); //set to false to have to call start //TODO set 'false' for flow study
  ...
  llvm::sys::Path LibraryDir(std::string(getenv("KLEE_HOME_DIR")) + "/lib");
//TODO -- changed this to find bad path for loading libkleeRuntimeIntrinsic.bca
...
//   llvm::sys::Path LibraryDir(KLEE_DIR "/lib");
// #endif
  ...
  std::vector<std::string> llvmLibSet;
  std::vector<std::string> nativeLibSet;

  handleSpecifiedExternLibs(llvmLibSet, nativeLibSet);
  // handle external libraries in LLVM format
  for (unsigned i = 0; i < llvmLibSet.size(); i++) {
    mainModule = klee::linkWithLibrary(mainModule, llvmLibSet[i]);
    assert(mainModule && "unable to link with llvm external library");
  }
  // handle external libraries in native format
  for (unsigned i = 0; i < nativeLibSet.size(); i++) {
    sys::DynamicLibrary::LoadLibraryPermanently(nativeLibSet[i].c_str());
  }
  ...
  // by Guodong: Segmentation fault?! 
  // (the problem is caused by deleting a solver more than one time)
  // Now fixed. However it may appear again when introducing new solvers
  delete interpreter;
